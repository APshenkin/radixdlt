...
public class SubmissionServiceTest {
	...
	public void testSubmitTx() throws Exception {
		...
		result
			.onFailureDo(Assert::fail)
			.flatMap(prepared -> signature.flatMap(recoverable -> {
				var id = TxLowLevelBuilder.newBuilder(prepared.getBlob())
					.sig(recoverable)
					.build().getId();

				return submissionService.submitTx(prepared.getBlob(), recoverable, id);
			}))
			.onFailureDo(Assert::fail)
			.onSuccess(Assert::assertNotNull);
	}
